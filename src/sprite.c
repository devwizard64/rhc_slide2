...
)
{
	...
	gDPSetTextureImage(glistp++, fmt, siz_load, tw >> shift, timg);
	for (ty = 0; ty < th; ty += ch)
	{
		...
		for (tx = 0; tx < tw; tx += cw)
		{
			...
			gDPPipeSync(glistp++);
			gDPSetTile(
				glistp++, fmt, siz_load, line, 0, 0, 0, 0, 0, 0, 0, 0, 0
			);
			gDPLoadSync(glistp++);
			gDPLoadTile(
				glistp++, 0,
			...
			gSPTextureRectangle(glistp++, xl, yl, xh, yh, 0, s, t, dsdx, dtdy);
		}
	}
}
