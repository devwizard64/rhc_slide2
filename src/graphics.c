...
static void cont_read(void)
{
	static const u16 button[] =
	{
		L_JPAD, R_JPAD, D_JPAD, U_JPAD, L_TRIG, R_TRIG, Z_TRIG, 0,
		A_BUTTON, B_BUTTON, 0, 0, START_BUTTON, 0, 0, 0,
	};
	int i, n;
	osRecvMesg(&si_mq, NULL, OS_MESG_BLOCK);
	osContGetReadDol(pad_status);
	for (i = 0; i < MAXCONTROLLERS; i++)
	{
		switch (pad_status[i].err)
		{
		case PAD_ERR_NONE:          cont_pad[i].errno = 0x00; break;
		case PAD_ERR_NO_CONTROLLER: cont_pad[i].errno = 0x80; break;
		case PAD_ERR_TRANSFER:      cont_pad[i].errno = 0x40; break;
		}
		if (cont_pad[i].errno) continue;
		cont_pad[i].button = 0;
		for (n = 0; n < 16; n++)
		{
			if (pad_status[i].button & (1 << n))
			{
				cont_pad[i].button |= button[n];
			}
		}
		if (pad_status[i].substickX < -50) cont_pad[i].button |= L_CBUTTONS;
		if (pad_status[i].substickX > +50) cont_pad[i].button |= R_CBUTTONS;
		if (pad_status[i].substickY < -50) cont_pad[i].button |= D_CBUTTONS;
		if (pad_status[i].substickY > +50) cont_pad[i].button |= U_CBUTTONS;
		cont_pad[i].stick_x = pad_status[i].stickX * 4/5;
		cont_pad[i].stick_y = pad_status[i].stickY * 4/5;
	}
}
#endif
...
static void gfx_ext(void)
{
	if (cont_bitpattern)
	{
		osContStartReadDol(&si_mq);
		osRecvMesg(&si_mq, NULL, OS_MESG_BLOCK);
		osContGetReadDol(pad_status);
	}
...
static void cont_init(void)
{
	...
	{
		int mask = 1 << i;
		if ((cont_bitpattern & mask) && cont_status[i].type != 9)
		{
			cont_bitpattern ^= mask;
		}
	}
...
void gfx_proc(UNUSED void *arg)
{
	...
	for (;;)
	{
		...
		if (cont_bitpattern) osContStartReadDol(&si_mq);
		aud_tick();
		frame_start();
		extflag = extproc();
		cont_proc();
