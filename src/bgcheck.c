...
#define MAP_MIN_X               (-30000)
#define MAP_MIN_Z               (-30000)
#define MAP_MAX_X               (+30000)
#define MAP_MAX_Z               (+30000)
	...
	while (list)
	{
		...
		else
		{
			...
			if (face->code == BG_123)
			{
				...
				{
					if (mario->flag & PL_00000002) continue;
				}
			}
		}
...
static BGFACE *bglist_check_roof(
	BGLIST *list, float x, float y, float z, float *roof_y
)
{
	register BGFACE *face;
	register float x0, z0, x1, z1, x2, z2;
	BGFACE *roof = NULL;
	float nx, ny, nz, nw;
	float level;
	roof = NULL;
...
static BGFACE *bglist_check_ground(
	BGLIST *list, float x, float y, float z, float *ground_y
)
{
	register BGFACE *face;
	register float x0, z0, x1, z1, x2, z2;
